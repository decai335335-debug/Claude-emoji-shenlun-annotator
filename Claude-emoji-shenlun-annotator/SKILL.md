...
即使严格串行,单会话累积的原文 + 标注内容会持续吃掉上下文 token,超过一定数量后会话整体卡死,与并行无关。实测:

- ✅ **1–5 个文件**:单会话可完整跑完。
- ⚠️ **6–9 个文件**:接近上限,可能中途卡。
- ❌ **≥ 10 个文件**:几乎必卡,表现为后期 Write 无响应/沙箱超时。

**强制策略**:

1. 用户一次投喂 **> 5 个**文件时,**立即停下来**,不要直接开始,先回复:
   > "检测到 N 个文件,超过单会话安全上限(5 个)。建议分批:
   > **第 1 批**:文件 1–5(本会话处理)
   > **第 2 批**:文件 6–N(请新开一个会话继续)
   > 是否按此执行?"
2. 得到确认后,**本会话只做前 5 个**,做完第 5 个就收尾汇报,**不要**继续第 6 个。
3. 收尾时明确提示用户:"已完成 5/10。剩余 5 个请**新开会话**投喂,否则会卡死。"
4. 禁止"我试试看能不能跑完 10 个" —— 实测红线就是红线,不要赌。

**为什么不能靠 /compact**:压缩丢失已标注段落的精确位置,断点续写会错位。不如换新会话干净。
